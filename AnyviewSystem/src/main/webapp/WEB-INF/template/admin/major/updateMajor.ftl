                ...
                <div class="ibox-title">
                    ...
                    <div class="ibox-tools">
                      <button type="button" class="btn btn-white" onclick="window.history.back();"
                            ...
                            <!-- <a class="btn btn-white" href="${base}/admin/major/MajorContent.jhtml" style="margin-top: -8px;">返回</a> -->
                    </div>
                </div>
                <div class="ibox-content">
                    <form id="saveMajorForm" class="form-horizontal" action="editSave.jhtml" method="post">
                        <input type="hidden" class="form-control" name="id" id="majorId" value="${major.id}">
                        <input type="hidden" name="majorUpdater" value="${systemUser.name}">
                        <div class="form-group">
                            <div class="row">
                                <div class="col-sm-6" >
                                    ...
                                    <!-- <input name="schoolName" id="schoolName" placeholder="请输入学校名称" class="form-control" value="${major.college.school.schoolName}">
                                    <input name="schoolId" id="schoolId" class="hidden" value="${major.college.school.id}"> -->
                                      <!-- <select  id="schoolId" onchange="add1(this.value);"
                                                class="bs-select form-control">

	                                            <option value="${major.college.school.id}" >${major.college.school.schoolName}</option>
		                                        [#list schoolLists as school]
		                                            <option value="${school.id}">${school.schoolName}</option>
		                                        [/#list]

	                                     </select> -->
                                       <select class="form-control" name="schoolId" id="schoolName" value="${major.college.school.id}" onchange="add1(this.value);">
                                          <option hassubinfo="true" value="${major.college.school.id}">${major.college.school.schoolName}</option>
                                       </select>
	                                 [/#if]
	                                 [#if systemUser.roleId=1]
		                                     <input type="hidden" >${systemUser.school.schoolName}
		                             [/#if]
                              ...
                              <div class="col-sm-6">
                                  ...
                                  <div class="col-sm-9">
                                     <select name="collegeId" id="collegeId" class="bs-select form-control">
                                    	 <option value="${major.college.id}">${major.college.collegeName}</option>
                        ...
                        <div class="form-group">
                            <div class="row">
	                            <div class="col-sm-6 vaild">
	                                    <label class="col-sm-3 control-label">专业名称<span class="required"> * </span>:</label>
	                                    <div class="col-sm-9">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#saveMajorForm").validate({
            submitHandler:function (form) {   //不使用submit默认提交功能，先进行自定义验证，成功则提交表单
                $.ajax({
                  url: "${base}/admin/major/check_majorName.jhtml",
                  type: "GET",
                  data: {
                    id: $("#majorId").val(),
